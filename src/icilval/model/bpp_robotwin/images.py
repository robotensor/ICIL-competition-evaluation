"""The image path for a checkpoint trained on 128-pixel renders, in numpy.

Ported from the benchmark adapter's `icil_policies/common/images.py`. The robot's arms sit
0.30 m either side of its centre line, so a centred square crop of a third-person frame puts the
acting arm near the image edge, where the checkpoint's training domain had it centred. The path:

1. `crop_square` cuts a square around a column - the projection of the acting arm's workspace
   centre, a constant of the camera and the embodiment, never scene information. From the
   third-person camera's 320x180 that is 180x180, and clamping keeps the column within 90-230.
...
3. `bilinear_resize` to 224 with `align_corners=False` and no antialiasing, which is
   `torch.nn.functional.interpolate(mode="bilinear", align_corners=False)` - how BPP brings its
   128-pixel training frames to 224.

`arm_centred_view` runs all three, so the model sees a 128-pixel frame brought to 224, as in
training, rather than a sharper 224 image. Both resizes are separable: one weight matrix per
axis, `rows @ image @ columns.T`.

Images are (h, w) or (h, w, channels) arrays; the resizes return float64 and
`arm_centred_view` float32 in [0, 1].
"""
...
#: The intermediate size: what the checkpoint's training frames were stored at.
MIDDLE = 128
#: What the network takes. The architecture template also declares it (`image_resolution`); this
#: is the default for a caller that has not been handed one.
OUTPUT = 224
...
def area_weights(n_in: int, n_out: int) -> np.ndarray:
    """(n_out, n_in): output pixel i averages the input interval [i s, (i + 1) s), s = n_in/n_out."""
    if n_out > n_in:
...
def arm_centred_view(
    ...
    With `quantize`, the 128-pixel stage is rounded to whole 0-255 levels, as a stored training
    frame was, before it is scaled to [0, 1] and resized - the order BPP's loader follows. Only
    uint8 frames are accepted: a float frame already in [0, 1] would come out near black.
    """
