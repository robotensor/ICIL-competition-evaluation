...
names the Hugging Face dataset and what to import from it, if it imports anything at all; how
is the skill's simulator's `build_stage` (`simulators/<sim>/pool.py`).
"""
...
def eligible_tasks(pool: Pool, skill: str) -> list[str]:
    """Tasks of a skill a unit can be drawn for: a prompt can be generated for them."""
    return sorted(t.task_id for t in pool.tasks.values() if t.skill == skill and t.valid_instances)
...
def finalize(pool: Pool, spec: Spec) -> dict[str, dict[str, int]]:
    ...
    pool.seal()
    pool.save()
    return {s: {"eligible": len(e["eligible"])} for s, e in pool.skills.items()}
