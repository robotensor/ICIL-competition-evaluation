...
(BPP's LIBERO-Gen Combination) and draw anything (DrawAnything-Sim). For every unit the model is
shown one demonstration of the task, **generated when the duel runs**, no language, and must do
the task on a scene that differs from the demonstration by one published change. Each skill is
one success rate; the final score is their mean; a challenger takes the crown by beating the
reigning model's average by `duel.score_margin` points on an identical unit list. Everything
published is signed; unit lists derive from `spec.json`, the catalogue id and the two model
references, and every generated prompt is published by hash.
